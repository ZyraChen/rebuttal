...
class ProQueryChain:
    ...
    def __init__(self, llm):
        ...
        self.first_round_template = PromptTemplate(
            ...
            template="""It is now 3 January 2026. You are a fact-checking expert for the affirmative side, responsible for finding supporting evidence for the following claim.
Claim: {claim}
...
Now please provide the queries (2-3 queries, one per line):"""
        ...
        self.followup_round_template = PromptTemplate(
            ...
            template="""You are a fact-checking expert for the affirmative side, responsible for finding supporting evidence for the following claim.
Claim: {claim}
    ...
    def generate_queries(
        ...
        if round_num == 1:
            try:
                ...
                print(f"  [ProRound1轮] 基于claim分解生成了 {len(queries)} queries（使用thinking模式）")
                return queries[:3]  # Round一轮最多返回3queries

            except Exception as e:
                print(f"  ⚠ Pro Chain (Round1轮) 调用failed: {e}")
                return []
        ...
        # 后续轮次：针对对手攻击进行反击
            ...
            try:
                ...
                print(f"  ⚠ Pro Chain (Round{round_num}轮) 调用failed: {e}")
                return []
